...
import de.goldendeveloper.dcbcore.MysqlConnection;
...
public class CoreEvents extends ListenerAdapter {
    ...
    public void onGuildLeave(@NotNull GuildLeaveEvent e) {
        try {
            dcBot.getServerCommunicator().removeServer(e.getGuild().getId());
            e.getJDA().getPresence().setActivity(Activity.playing("/help | " + e.getJDA().getGuilds().size() + " Servern"));
        } catch (Exception exception) {
            exception.printStackTrace();
            Sentry.captureException(exception);
    ...
    public void onGuildMemberJoin(@NotNull GuildMemberJoinEvent e) {
        try {
            if (dcBot.getMysqlConnection().getMysql().existsDatabase(MysqlConnection.dbName)) {
                if (dcBot.getMysqlConnection().getMysql().getDatabase(MysqlConnection.dbName).existsTable(MysqlConnection.settingsTName)) {
                    Table table = dcBot.getMysqlConnection().getMysql().getDatabase(MysqlConnection.dbName).getTable(MysqlConnection.settingsTName);
